...
let _anlSearch = '';             // Student Performance Matrix name/ID filter
let _anlPage = 1;                // Student Performance Matrix current page
...
window.anlGoToPage = function (page) {
  ...
  _anlRenderMatrix();
  const table = document.getElementById('anl-matrix');
...
/** Re-renders just the Student Performance Matrix section (search/page changes don't need a full page repaint). */
function _anlRenderMatrix() {
  const el = document.getElementById('anl-matrix');
  if (!el) return;
  ...
  if (!filtered.length) {
    el.innerHTML = `<div style="text-align:center;padding:32px;color:var(--text-muted);font-size:13px">No students match "${_esc(_anlSearch)}".</div>`;
    return;
  }
  ...
  // Rank (#) reflects standing in the full sorted roster, not the filtered/paged view.
  ...
  el.innerHTML = `
    <table class="admin-table">
      ...
      <tbody>
        ${shown.map(s => `<tr>
          <td style="font-family:var(--fm);font-size:10px;color:var(--text-muted)">${String(rankOf(s)).padStart(2, '0')}</td>
...
window.renderAnalytics = function() {
  ...
  <div style="margin-bottom:12px;max-width:280px">
    <input type="text" placeholder="Search by name or ID…" value="${_esc(_anlSearch)}" oninput="anlSetSearch(this.value)">
  </div>
