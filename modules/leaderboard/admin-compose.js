...
//             shared/utils.js (uid),
//             core/state-manager.js (AppStore — titles slice, for title-grant rewards).
...
window.renderAdminMail = function(){
  ...
  const mails = [...(AppStore.getSlice(s => s.mail) || [])].sort((a,b)=>new Date(b.sentAt)-new Date(a.sentAt));
  const totalSent = mails.length;
  const withRewards = mails.filter(m=>m.hasReward).length;
  const totalRecipients = mails.reduce((s,m)=>s+(m.to==='all'?students.length:(m.to||[]).length),0);
  ...
  mails.map(m=>{
    const recipientLabel = m.to==='all'?`All Students (${students.length})`
      :Array.isArray(m.to)?m.to.map(sid=>{const s=students.find(x=>x.id===sid);return s?s.name:sid;}).join(', ')
      :m.to;
    ...
    const claimsCount = Array.isArray(m.to)?m.to.filter(sid=>mailIsClaimed(m,sid)).length
      :m.to==='all'?students.filter(s=>mailIsClaimed(m,s.id)).length:0;
    const readCount = Array.isArray(m.to)?m.to.filter(sid=>mailIsRead(m,sid)).length
      :m.to==='all'?students.filter(s=>mailIsRead(m,s.id)).length:0;
    const totalTo = m.to==='all'?students.length:Array.isArray(m.to)?m.to.length:1;
...
window.mailAdminOpenCompose = function(editId){
  const students = AppStore.getSlice(s => s.students) || [];
  const existing = editId ? (AppStore.getSlice(s => s.mail) || []).find(m=>m.id===editId) : null;
  const studentOpts = students.map(s=>`<option value="${s.id}" ${existing&&Array.isArray(existing.to)&&existing.to.includes(s.id)?'selected':''}>${_esc(s.name)}</option>`).join('');
  const typeOpts = MAIL_TYPES.map(t=>`<option value="${t.value}" ${(existing?.type||'general')===t.value?'selected':''}>${t.label}</option>`).join('');
  const titles = (AppStore.getSlice(s => s.titles) || []).filter(t=>t.active);
  ...
  const recipientLabel = existing
    ? (existing.to === 'all' ? `All Students (${students.length})`
        : (existing.to || []).map(sid => { const s = students.find(x => x.id === sid); return s ? s.name : sid; }).join(', '))
    : '';
...
window.mailAdminSend = async function(editId){
  ...
  if(!editId){
    ...
    if(toSel==='all'){
      to = (AppStore.getSlice(s => s.students) || []).map(s=>s.id); // send_mail() needs explicit recipient ids — there's no server-side "all" concept, just one row per recipient
    } else {
  ...
  if(editId){
    ...
    // [Phase 3 migration bugfix] The pre-migration version mutated the
    // local DB.mail[idx] here but never persisted it (no saveDB() call in
    // this branch, unlike the "compose new" branch below) — and
    // renderAdminMail(), called two lines down, reloads from persisted
    // storage as its very first line. So the local edit was silently
    // discarded the instant the RPC succeeded: the admin's screen would
    // keep showing the pre-edit content until the next real Supabase pull
    // resynced it, even though the server-side update had already gone
    // through. AppStore.updateState() always persists, so this local view
    // now reflects the edit immediately, matching what the RPC just did.
    AppStore.updateState(draft => {
      if (!Array.isArray(draft.mail)) return;
      const idx = draft.mail.findIndex(m=>m.id===editId);
      if(idx>=0){
        const existing = draft.mail[idx]||{};
        draft.mail[idx] = {...existing, subject, body, type, hasReward, rewards, icon: mailTypeIcons[type]||'📬'};
      }
    }, { type: 'mail:updated', payload: { mailId: editId } });
    ...
    }, { type: 'mail:sent', payload: { mailId: newMail.id, recipientCount: to.length } });
    closeModalForce();
    renderAdminMail();
...
window.mailAdminConfirmDelete = async function(mailId){
  AppStore.updateState(draft => {
    draft.mail = (draft.mail||[]).filter(m=>m.id!==mailId);
  }, { type: 'mail:deleted', payload: { mailId } });
  closeModalForce();
